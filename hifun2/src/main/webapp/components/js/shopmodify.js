...
$(document).ready(function(){
	...
	$("#submit").click(function(){
		...
		ajaxPost(
				$("#base").val()+'/headpage/submitshopmodify.do', 
				$("#mainForm").serialize(), 
				'json', doShopmodifySuccess
				);
		return false;
	});
});
...
function doQueryShopTypeSuccess(res){
	...
	for(var i = 0;i < data.length;i++){
		if(data[i].typeId == $("#shopTypeHid").val()){
			type = type + "<option value='" + data[i].typeId + "' selected='selected'>" + data[i].typeName + "</option>";
		}else{
...
function doQueryShopLevelSuccess(res){
	...
	for(var i = 0;i < data.length;i++){
		if(data[i].typeId == $("#shopLevelHid").val()){
			type = type + "<option value='" + data[i].typeId + "' selected='selected'>" + data[i].typeName + "</option>";
		}else{
			type = type + "<option value='" + data[i].typeId + "'>" + data[i].typeName + "</option>";
		}
...
function doShopmodifySuccess(res){
	var data = res.data;
	if(data == 0){
		...
		window.wxc.xcConfirm('修改成功，'+second+'秒后自动跳转', window.wxc.xcConfirm.typeEnum.success);
		setTimeout(function(){
