...
public class SignCalcQuartz implements Job {
    ...
    public synchronized void work() {
        ...
        for (String username : signMap.keySet()) {
            System.out.println(username + "->" + signMap.get(username));
            headService.updateUserSignByUsername(username,
                (Integer) signMap.get(username));
        }
    }
